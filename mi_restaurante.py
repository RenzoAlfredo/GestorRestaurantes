...
def total():
    ...
    var_impuestos.set(f'S/. {round(impuestos, 2)}')
    ...
    for bebida in  texto_bebida:
        if bebida.get() != '0':
            texto_recibo.insert(END, f'{lista_bebidas[x]}\t\t{bebida.get()}\t'
                                     f'S/. {int(bebida.get()) * precios_bebida[x]}\n')
        x += 1
    ...
    for postre in  texto_postre:
        if postre.get() != '0':
            texto_recibo.insert(END, f'{lista_postres[x]}\t\t{postre.get()}\t'
                                     f'S/. {int(postre.get()) * precios_postres[x]}\n')
        x += 1
...
def guardar():
    ...
    archivo.close()
    messagebox.showinfo('Informaciòn', 'Su recibo ha sido guardado')
...
# tamaño de la ventana, dimensiones
...
aplicacion.title("Mi restaurante - Sistema de facturaciòn")
